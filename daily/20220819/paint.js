...
function drawBrushMarks(globalSettings, allPolySettings, chaikinPoly = false) {
  c.push()
  c.noStroke()
  c.colorMode(HSB)
  ...
  allPolySettings.forEach((settings) => {
    layers.push(
      new Array(settings.nLayers).fill()
        .map((x, i) =>
          deform(
            poly(settings.polyRadius, noise(i) * settings.noiseScale),
            settings.polyDeformN,
            settings.polyDeformVar))
    )
  })
  ...
  for (let i = 0; i < totalLayers; i++) {
    ...
    let thisLayer = polyLayer[i % polyLayer.length]
    let finalLayer = chaikinPoly ? chaikin(thisLayer) : thisLayer
...
function chaikin(points) {
  ...
  for (var i = 0; i < points.length - 1; i++) {
    const a = createVector(...points[i]);
    const b = createVector(...points[i + 1])


    // a*0.75 + b*0.25
    newPoints.push(a.mult(0.5).add(b.mult(0.5)).copy().array())

    // a*0.25 + b*0.75
    newPoints.push(a.mult(0.5).add(b.mult(0.5)).copy().array())

    // newPoints.push(a.lerp(b, 0.75).copy().array())
    // newPoints.push(b.lerp(a, 0.75).copy().array())
