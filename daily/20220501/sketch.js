...
class Soma {
  constructor(x, y, w, type) {
    ...
    this.type = type ? type : random([0, 1])
  }
  ...
  display() {
    if (this.type == 0) {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        let xx = this.x + this.d * cos(a);
        let yy = this.y + this.d * sin(a);
        vertex(xx, yy)
      }
      endShape(CLOSE)
    } else if (this.type == 1) {
      ...
      let y1 = this.y - this.d
      let a = random(2, 6)
      for (let i = y1; i < y2; i++) {
        let xx = map(i, y1, y2, 0, TAU)
        let yy = i;
        vertex(
          sin(xx + a) * a,
          yy)
